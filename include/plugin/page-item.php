	...
	*/
	...
	function print_testimonial($item_xml){
		...
		if($display_type == 'Specific Testimonial'){
			...
			$posts = get_page_by_title( $specific, OBJECT, 'testimonial' );
			...
			$position = __(get_post_meta($posts->ID, 'testimonial-option-author-position', true), 'gdl_front_end');
			
			echo '<div class="testimonial-author gdl-divider">';
			echo '<span class="testimonial-author-name">' . $posts->post_title . '</span>';
			if( !empty( $position ) ){
